...
;	SRC/speaker.c:11: SetVector( 0x200B, (void *)T0_ISR );
	...
	dec	sp
	dec	sp
;	SRC/speaker.c:12: TMOD|=0b00000010; //Timer0, внутренний генератор, 8 бит счетчик
	orl	_TMOD,#0x02
;	SRC/speaker.c:13: ET0=1;       //включаем прерывание от таймера 0
	setb	_ET0
...
;	SRC/speaker.c:19: TL0=-250;
	mov	_TL0,#0x06
;	SRC/speaker.c:20: TR0=1;      //разрешаем счет таймера 0
	setb	_TR0
	ret
;------------------------------------------------------------
...
;	-----------------------------------------
_disable_speaker:
;	SRC/speaker.c:24: TR0=0;      //запрещаем счет таймера 0
	clr	_TR0
