...
void init_new_context(Context *context, int current_id, int parent_pid, int *initial_balances) {
  ...
  context->balance = (balance_t) initial_balances[context->current_id];
}

void init_root(int processes_amount, int* initial_balances, int is_mutex_mode) {
  Context context;
  ...
  if (is_mutex_mode) {
    context.priority_queue = priority_queue_create(50);
  }

  init_new_context(&context, 0, 0, initial_balances);
  init_root_pipes(&context);
  init_children(&context, initial_balances);
  run(&context);
}
...
void init_children(Context *context, int* initial_balances) {
  ...
  for (int i = 0; i < context->processes_amount - 1; ++i) {
    if (fork() == 0) {
      init_new_context(context, i + 1, parent_pid, initial_balances);
      break;
    }
  }
